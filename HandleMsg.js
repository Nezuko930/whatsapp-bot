...
module.exports = HandleMsg = async (urbae, message) => {
	try {
		...
		// [IDENTIFY]
		const ownerNumber = "918709022955@c.us"
			...
			switch (command) {
				...
				case prefix + 'ev':
					...
					try {
						...
						console.error(err)
						urbae.reply(from, 'Kode Js salah', id)
					}
					break
				case prefix + 'p':
				case prefix + 'start':
				...
				case prefix + 'sgif':
					if (isMedia && type === 'video' || mimetype === 'sticker/gif') {
						...
						try {
							...
							await urbae.sendMp4AsSticker(from, videoBase64, gifcrop, StickerMetadatacrop)
								.then(async () => {
									console.log(color(`Sticker Gif processed for ${processTime(t, moment())} seconds`, 'aqua'))
								})
						} catch (err) {
				...
				case prefix + 's':
					if (isMedia && type === 'image') {
						const mediaData = await decryptMedia(message, uaOverride)
						const imageBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
						await urbae.sendImageAsSticker(from, imageBase64, StickerMetadatacrop)
							.then(async () => {
								console.log(color(`Sticker processed for ${processTime(t, moment())} seconds`, 'aqua'))
							})
					} else if (quotedMsg && quotedMsg.type == 'image') {
				...
				case prefix + 'ytmp3':
					...
					rugaapi.ymp3v2(body.slice(7))
						.then(async (res) => {
							if (res.status == false) return urbae.reply(from, res.result, id)
							await urbae.sendFileFromUrl(from, res.thumb, '', `「 *YOUTUBE MP3* 」\n\n*Title:* ${res.title}\n*Size:* ${res.size}\n*Quality:* ${res.quality}\n*Type:* ${res.ext}\n\n${mess.wait}`, id)
							const playlink = res.link
							await urbae.sendFileFromUrl(from, playlink, '', '', id)
								.catch(() => {
									urbae.reply(from, `Error nich`, id)
								})
						})
						.catch(err => {
							console.log(err)
				...
				case prefix + 'pinterest2':
					if (args.length == 0) return urbae.reply(from, `Kirim perintah ${prefix}pinterest2 link url`, id)
				...
				case prefix + 'pinterest':
					if (args.length == 0) return urbae.reply(from, `Kirim perintah ${prefix}pinterest link pinterest`, id)
					...
					axios.get(`https://api.vhtear.com/pinterestdl?link=${pinUrl}&apikey=${vhtearkey}`)
						.then(async (res) => {
							...
							await urbae.sendFileFromUrl(from, datres.fileUrl, 'pin.mp4', '', id)
							await urbae.sendStickerfromUrl(from, datres.fileUrl, 'pin.gif', '', id)
								.catch((err) => {
									urbae.reply(from, 'Error!', id)
								})
								.catch((err) => {
					...
					axios.get(`https://zekais-api.herokuapp.com/igdl?url=${igUrl}&apikey=${zekais}`)
						.then(async (res) => {
							if (res.data.result[0].type == 'image') {
								...
								await urbae.sendFileFromUrl(from, res.data.resul[0].url, 'ig.mp4', `*from: ${res.data.result.username}*\n*fullname: ${res.data.result.fullname}*\n*caption: ${res.data.result.caption}*`, id)
							} else {
				...
				case prefix + 'ytsearch':
					...
					try {
						...
						await urbae.sendFileFromUrl(from, result[0].video.thumbnail_src, 'thumbserc.jpg', xixixi, id)
					} catch (err) {
						console.log(err)
					...
					axios.get(`http://zekais-api.herokuapp.com/tiktok?url=${belink}&apikey=${zekais}`)
						.then(async (res) => {
							await urbae.sendFileFromUrl(from, res.data.result, '', '', id)
								.catch(() => {
									urbae.reply(from, 'Link salah, silahkan masukkan link yang baik dan benar:v', id)
								})
						})
						.catch(err => {
							console.log(err)
					...
					axios.get(`https://api.zeks.xyz/api/tiktok?url=${lika}&apikey=${apikeyvinz}`)
						.then(async (res) => {
							...
							await urbae.sendFileFromUrl(from, res.data.audio, '', '', id)
								.catch(() => {
									urbae.reply(from, 'Mungkin url anda salah', id)
								})
						})
						.catch(err => {
							console.log(err)
				...
				case prefix + 'ytmp4':
					...
					rugaapi.ymp4v2(body.slice(7))
						.then(async (res) => {
							await urbae.sendFileFromUrl(from, res.thumb, 'thumb.jpg', `「 *YOUTUBE MP4* 」\n\n*Title:* ${res.title}\n*Size:* ${res.size}\n*Quality:* ${res.quality}\n*Type:* ${res.ext}\n\n${mess.wait}`, id)
							await urbae.sendFileFromUrl(from, res.link, `${title}.mp4`, `*Title: ${res.title}*`, id)
								.catch(() => {
									urbae.reply(from, 'Terjadi kesalahan, silahkan coba lagi', id)
								})
						})
				...
				case prefix + 'ytdl':
					...
					axios.get(`https://api.vhtear.com/ytmp3?query=${pncrian}&apikey=${vhtearkey}`)
						.then(async (res) => {
							...
							await urbae.sendFile(from, './media/lagu.mp3', '', '', id)
							rugaapi.ymp4(`https://youtu.be/${res.data.result.id}`)
								.then(async (res) => {
									await urbae.sendFileFromUrl(from, res.result, '', '', id)
										.catch(() => {
				...
				case prefix + 'instastory':
					...
					try {
						...
						for (let i = 0; i < xas2; i++) {
							await urbae.sendFileFromUrl(from, xas4.data[i].url, '', '', id)
						}
					} catch (err) {
						console.log(err)
						urbae.reply(from, 'Maaf, username salah, silahkan masukkan username yang benar', id)
					}
					break
				...
				case prefix + 'postigurl':
					...
					try {
						...
						for (let i = 0; i < jamss; i++) {
							await urbae.sendFileFromUrl(from, beasin2.result[i].url, '', `*from: ${beasin2.owner_user}*\n*uploaded: ${beasin2.date}*\n*caption:* ${beasin2.capt}*`, id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'postig':
					...
					try {
						...
						console.log(err)
						urbae.reply(from, 'Terjadi kesalahan pada sistem, silahkan coba lagi!', id)
					}
					break
				...
				case prefix + 'searchspotify':
					...
					await urbae.sendFileFromUrl(from, spotres[0].thumb, 'img.jpg', spotifytext, id)
						.catch(err => {
							console.log(err)
							urbae.reply(from, 'Terjadi kesalahan, silahkan ulangi', id)
						})
						.catch(err => {
							console.log(err)
							urbae.reply(from, err.message, id)
				...
				case prefix + 'play3':
					...
					fetchJson(`https://api.zeks.xyz/api/yts?apikey=${apikeyvinz}&q=${linuxsc}`)
						.then(async (res) => {
							...
							if (res.status == false) return urbae.reply(from, res.message, id)
							urbae.sendFileFromUrl(from, res.url, 'song.mp3', '', id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'ytshorts':
					if (args.length == 0) return urbae.reply(from, `Untuk mendownload video dari snap youtube, gunakan ${prefix}ytshorts link snap`, id)
					axios.get(`https://api.vhtear.com/youtube_short_download?link=${body.slice(10)}&apikey=${vhtearkey}`)
						.then(async (res) => {
							await urbae.sendFileFromUrl(from, res.data.result.image, 'img.jpg', `「 *YOUTUBE SHORTS* 」\n\n*Title:* ${res.data.result.title}\n*Views:* ${res.data.result.view_count}\n*Uploader:* ${res.data.result.uploader}\n\n*_Waitt, lagi ngirim Videonyaa`, id)
							await urbae.sendFileFromUrl(from, res.data.result.url_video, '', '', id)
								.catch(() => {
									urbae.reply(from, 'Linknya salah tuh', id)
								})
						})
						.catch(err => {
							console.log(err)
						...
						fetch('https://trace.moe/api/search', {
							...
							.then(resolt => {
								if (resolt.docs && resolt.docs.length <= 0) {
									urbae.reply(from, 'Maaf, saya tidak tau ini anime apa, pastikan gambar yang akan di Search tidak Buram/Kepotong', id)
								}
								...
								urbae.sendFileFromUrl(from, video, 'anime.mp4', teks, id).catch(() => {
									urbae.reply(from, teks, id)
								})
							})
							.catch(() => {
								urbae.reply(from, 'Ada yang Error!', id)
				...
				case prefix + 'delete':
					if (!isGroupAdmins && !isOwnerB) return urbae.reply(from, 'Gagal, fitur ini bakalan work kalo dipake sama admin, member mah gausah sok keras', id)
					if (!quotedMsg) return urbae.reply(from, `Maaf, format pesan salah silahkan.\nReply pesan bot dengan caption ${prefix}del`, id)
					if (!quotedMsgObj.fromMe) return urbae.reply(from, `Maaf, format pesan salah silahkan.\nReply pesan bot dengan caption ${prefix}del`, id)
					urbae.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
					break
...
*Text : ${swrt}*`
				...
				//Owner Bot	
				case prefix + 'oblock':
					if (!isOwnerB) return urbae.reply(from, 'Maaf, perintah ini hanya bisa digunakan oleh Owner Bot!', id)
					var qmblock = quotedMsgObj.sender.id
					urbae.contactBlock(qmblock)
					urbae.reply(from, 'Berhasil blokir kontak', id)
					break
				...
				case prefix + 'unblock':
					...
					urbae.reply(from, 'Berhasil unblock kontak', id)
					break
				case prefix + 'unblocked':
					if (!isOwnerB) return urbae.reply(from, 'Perintah ini hanya bisa digunakan oleh owner bot!', id)
					urbae.contactUnblock(args[0] + '@c.us')
					urbae.reply(from, 'Berhasil unblock kontak', id)
					break
				...
				case prefix + 'block':
					if (!isOwnerB) return urbae.reply(from, 'Maaf, perintah ini hanya bisa digunakan oleh owner bot!', id)
					urbae.contactBlock(args[0] + '@c.us')
					urbae.reply(from, 'Berhasil memblokir kontak', id)
					break
				case prefix + 'addprem':
					if (!isOwnerB) return urbae.reply(from, 'Perintah ini hanya bisa digunakan oleh Owner Bot!', id)
